...
{
  uint8_t IIRValue, LSRValue;				// local auto var
  uint8_t Dummy = Dummy;
  ...
  {
	/* Receive Data Available */
	UART0Buffer[UART2Count] = LPC_UART2->RBR;
	UART2Count++;
	if ( UART2Count == BUFSIZE )
	{
	  UART2Count = 0;		/* buffer overflow */
...
uint32_t UARTInit( uint32_t PortNum, uint32_t baudrate )
{
  ...
  else if ( PortNum == 1 )
  {
	...
	//LPC_PINCON->PINSEL4 |= 0x0000000A;	/* Enable RxD1 P2.1, TxD1 P2.0 */
	  ...
	  LPC_PINCON->PINSEL0 &= ~0xC0000000;    /*P0.15 is TXD1*/
	  LPC_PINCON->PINSEL0 |=  0x40000000;

	  LPC_PINCON->PINSEL1 &= ~0x00000003;    /*P0.16 is RXD1*/
  ...
  }else if ( PortNum == 2 )
  {
	...
	//LPC_PINCON->PINSEL4 |= 0x0000000A;	/* Enable RxD1 P2.1, TxD1 P2.0 */
	  ...
	  LPC_PINCON->PINSEL0 &= ~0x00F00000;    /*P0.11 is RX2 and P.10 is TX2*/
