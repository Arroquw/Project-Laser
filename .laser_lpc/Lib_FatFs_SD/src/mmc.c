...
DSTATUS disk_initialize(BYTE drv /* Physical drive nmuber (0) */
	...
	if (drv)
		return STA_NOINIT; /* Supports only single drive, drv could only be zero */
...
BYTE count /* Sector count (1..255) */
	...
	if (drv || !count)
		return RES_PARERR;				//count must be non-zero
	if (Stat & STA_NOINIT)
		return RES_NOTRDY;
...
int sendCMD0() {
	...
	useSPI(0x00);
	useSPI(0x00);
	useSPI(0x95); //CMD0 command "40 00 00 00 00 95"
...
void useSPI(unsigned char data) {
	...
	S0SPDR = data;					//send data CMD CODE
	//Data adress what to read
	// CMD17 for single block
	// CMD18 for multi block
	// block = 512 bytes
...
void init_SPI() {
	...
	S0SPCR |= (1 << 5) | (1 << 8);	//control register. enable master and 16 bit mode.
	select();
}
